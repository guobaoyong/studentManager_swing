...
public class Manage extends JInternalFrame {
	...
	private void TableDeleteActionEvent(ActionEvent e) {
		...
		if(n==0){
			try{
				int deleteNum=studentDao.delete(id);
				if(deleteNum==1){
	...
	private void TableUpdateActionEvent(ActionEvent e) {
		...
		try{
			int modifyNum=studentDao.update(stu);
			if(modifyNum==1){
	...
	private void TableMousePressed(MouseEvent e) {
		...
		ageTxt.setText(String.valueOf(table.getValueAt(row, 4)));
	}
...
//		Connection con=null;
		try{
//			con=dbUtil.getCon();
			List<Student> lis=studentDao.list(student);
			for (Student student2 : lis) {
				Vector v=new Vector();
				v.add(student2.getId());
				v.add(student2.getNumber());
				v.add(student2.getName());
				v.add(student2.getSex());
